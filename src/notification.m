...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
  ...
  NSString *url =
      response.notification.request.content.userInfo[@"DELETION_URL"];
...
void uploader_init() {
  ...
  UNNotificationAction *delete_action = [UNNotificationAction
      actionWithIdentifier:@"DELETE_FILE"
                     title:@"Delete"
                        ...
                        [center
                            addNotificationRequest:request
                             withCompletionHandler:^(NSError *_Nullable error) {
                               if (error)
                                 NSLog(@"notification delivery failed: %@",
                                       error);
                             }];
                      }];
  }
...
void error_notification(char *title, char *body) {
  display_notification(title, body, NULL);
}
