...
JSONElement *_JSON_parse(char *data, size_t *cursor_ptr) {
		...
		while (data[(*cursor_ptr)] != '}') {
			...
			if (data[(*cursor_ptr)] == ',') {
				...
				if (data[(*cursor_ptr)] == '}') {
					...
					printf("format error at cursor pos: %zu, expected `,` OR "
						   "`}`, but "
						   "found `%c`\n",
	...
	if (isdigit(data[(*cursor_ptr)])) {
		...
		if (endptr == data + (*cursor_ptr)) {
			printf(
				"format error at cursor pos: %zu, expected a number (double)\n",
				(*cursor_ptr));
			free(element);
			return NULL;
		}
			...
			if (endptr == walker) {
				if (element->JSONType == JSONType_OBJECT) {
					...
					if (i == element->child_count) {
						return JSON_get(end,
										get_element_from_object(key, element));
					} else {
						if (element->value.object[i]->JSONType !=
							JSONType_ARRAY) {
							return JSON_get(
								end, get_element_from_object(key, element));

						} else {
							if (index >=
								element->value.object[i]->child_count) {
								return JSON_get(
									end, get_element_from_object(key, element));
							} else {
								...
								return JSON_get(
									...
									element->value.object[i]->value.array[i]);
							}
						}
					}
				}
